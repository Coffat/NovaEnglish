...
public class AttendanceDetailSidePanel extends JPanel {
    ...
    private Consumer<Attendance> onSaveCallback;
    ...
    public AttendanceDetailSidePanel(Runnable onCloseCallback) {
        this.onCloseCallback = onCloseCallback;
        initUI();
    }

    public void setAttendance(Attendance attendance, Consumer<Attendance> onSaveCallback) {
        ...
        if (attendance != null) {
            ...
            if (attendance.getStudent() != null) {
                for (int i = 0; i < cbStudent.getItemCount(); i++) {
                    if (cbStudent.getItemAt(i).getId() == attendance.getStudent().getId()) {
            ...
            // Find and set class
            if (attendance.getCourseClass() != null) {
                for (int i = 0; i < cbCourseClass.getItemCount(); i++) {
                    if (cbCourseClass.getItemAt(i).getId() == attendance.getCourseClass().getId()) {
                        cbCourseClass.setSelectedIndex(i);
                        break;
                    }
                }
            }
            tfDate.setText(attendance.getAttendanceDate() != null ? attendance.getAttendanceDate().toString() : LocalDate.now().toString());
    ...
    private void onSave() {
        if (onSaveCallback != null) {
            ...
            try {
                ...
                currentAttendance.setAttendanceDate(LocalDate.parse(dateText, DateTimeFormatter.ofPattern("yyyy-MM-dd")));
            } catch (Exception e) {
                JOptionPane.showMessageDialog(this, "Invalid Date format. Use yyyy-MM-dd.", "Error", JOptionPane.ERROR_MESSAGE);
                return;
            }

            currentAttendance.setStudent(selectedStudent);
            currentAttendance.setCourseClass(selectedClass);
            currentAttendance.setStatus((String) cbStatus.getSelectedItem());

            onSaveCallback.accept(currentAttendance);
        }
    }
