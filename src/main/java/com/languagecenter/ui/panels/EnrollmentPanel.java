...
public class EnrollmentPanel extends JPanel {
    ...
    public EnrollmentPanel(com.languagecenter.ui.MainFrame mainFrame) {
    ...
    private void initUI() {
        ...
        String[] columns = {"ID", "Student", "Class", "Date", "Status", "Result", "Actions"};
                ...
                if (row < table.getRowCount() && row >= 0 && column < table.getColumnCount() && column >= 0) {
                    if (table.getColumnName(column).equals("Actions")) {
                        ...
                        if (clickX < cellRect.width / 2) {
                            ...
                            if (enrollment != null && mainFrame != null) {
                                mainFrame.openEnrollmentSidePanel(enrollment, (updatedEnrollment) -> {
                                    enrollmentDAO.updateEnrollment(updatedEnrollment);
                                    loadData();
                                    mainFrame.toggleSidePanel(false);
                                    JOptionPane.showMessageDialog(mainFrame, "Enrollment updated successfully!");
                                });
                            }
                        } else {
                            // Delete
                            int confirm = JOptionPane.showConfirmDialog(EnrollmentPanel.this,
        ...
        btnAdd.addActionListener(e -> {
            if (mainFrame != null) {
                mainFrame.openEnrollmentSidePanel(null, (newEnrollment) -> {
                    enrollmentDAO.addEnrollment(newEnrollment);
                    loadData();
                    mainFrame.toggleSidePanel(false);
                    JOptionPane.showMessageDialog(mainFrame, "Enrollment added successfully!");
                });
            }
        });
    ...
    private void loadData() {
        ...
        for (Enrollment e : enrollments) {
            String studentName = e.getStudent() != null ? e.getStudent().getFullName() : "N/A";
            String className = e.getCourseClass() != null ? e.getCourseClass().getClassName() : "N/A";
            Object[] row = new Object[]{
                    e.getId(),
                    studentName,
                    className,
                    e.getEnrollmentDate() != null ? e.getEnrollmentDate().toString() : "",
                    e.getStatus() != null ? e.getStatus() : "N/A",
    ...
    class StatusBadgeRenderer extends DefaultTableCellRenderer {
        ...
        public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
            ...
            if (value != null) {
                ...
                if (value.toString().equalsIgnoreCase("Confirmed")) {
                    badge.setForeground(new Color(0x15803D));
                    pill.setBackground(new Color(0xDCFCE7));
                    pill.setOpaque(true);
                } else if (value.toString().equalsIgnoreCase("Cancelled")) {
